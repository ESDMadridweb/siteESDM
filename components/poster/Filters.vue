<template>
    ...
    <div class="bg-green fixed bottom-0 left-0 w-full p-6 flex flex-col gap-6 z-50 transition-all duration-1000 ease-in-out" :class="{ 'translate-y-full' : !isOpen }">
        <div class="w-full flex justify-between text-T05">
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-3 text-T04 lg:text-T03">
                <template v-for="tema in data?.temas">
                    <span v-if="tema?.count > 0"  class="flex items-start opacity-40 cursor-pointer select-none" @click="handleTemas(tema?.slug)" :class="{ '!opacity-100' : posterStore.temas.includes(tema.slug) }">
                        <span>{{ tema?.name }}</span>
                        <sup class="text-BO4">{{ tema?.count }}</sup>
                    </span>
                </template>
        ...
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-32">
            <div>
                <h4 class="text-LM02 uppercase font-mono">Tipos</h4>
                <div class="flex gap-2 text-BO2 lg:text-T05">
            ...
            <div>
                <h4 class="text-LM02 uppercase font-mono">Fechas</h4>
                <div class="flex gap-2 text-BO2 lg:text-T05">
